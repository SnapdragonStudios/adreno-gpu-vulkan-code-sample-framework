...
#define SHADER_SPECMAP_TEXTURE_LOC          4
...
layout(set = 0, binding = SHADER_SPECMAP_TEXTURE_LOC) uniform sampler2D              u_SpecMapTex;
...
//-----------------------------------------------------------------------------
{
    ...
    // Get specular value(s) from the SpecMap texture
    vec4 SpecMapValue = texture( u_SpecMapTex, v_TexCoord.xy );
    ...
    // Write out the color and put specular in the Alpha
    FragColor = vec4(AlbedoColor.rgb, SpecMapValue.g);
