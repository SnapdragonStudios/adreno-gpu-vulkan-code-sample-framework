...
#define SHADER_DEPTH_SHADOW_TEXTURE_LOC     3
...
layout(set = 0, binding = SHADER_DEPTH_SHADOW_TEXTURE_LOC) uniform sampler2D u_ShadowMap;   // 'traditional' rasterized shadow depth map
...
// Shadow code
#define USE_VARIANCE_SHADOWMAP
#define SHADOW_VARIANCE_DISTANCE_ATTEN_DISABLE (1)  /* no shadow distance fade */
...
//-----------------------------------------------------------------------------
{
    ...
    float ShadowAtten = GetInShadowVSM(ShadowPos);
    ...
    // Directional light
    {
        float LightAng = max(0.0, -dot( FragCB.DirectionalLightDirection.xyz, Normal));
        LightAmt += ShadowAtten * LightAng * FragCB.DirectionalLightColor.rgb;
    }
