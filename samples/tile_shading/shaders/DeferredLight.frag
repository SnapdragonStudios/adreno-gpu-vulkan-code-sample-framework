...
//-----------------------------------------------------------------------------
{
    ...
    uint currentClusterIndexX = uint(floor((v_TexCoord.x * screenSize.x) / float(sceneInfo.clusterSizeX)));
    uint currentClusterIndexY = uint(floor((v_TexCoord.y * screenSize.y) / float(sceneInfo.clusterSizeY)));

    uint clusterIndex = currentClusterIndexY * sceneInfo.globalClusterCountX + currentClusterIndexX;
