...
//-----------------------------------------------------------------------------
{
    ...
    // Texture Colors
    // ********************************
    // Load all 4 MSAA samples at once
    vec4 DiffuseColor0 = subpassLoad( u_DiffuseInput, 0 );
    vec4 DiffuseColor1 = subpassLoad( u_DiffuseInput, 1 );
    FragColor.rgb = ACESFilmic(DiffuseColor0.rgb);
    vec4 DiffuseColor2 = subpassLoad( u_DiffuseInput, 2 );
    FragColor.rgb += ACESFilmic(DiffuseColor1.rgb);
    vec4 DiffuseColor3 = subpassLoad( u_DiffuseInput, 3 );
    FragColor.rgb += ACESFilmic(DiffuseColor2.rgb);
    FragColor.rgb += ACESFilmic(DiffuseColor3.rgb);

    // Average for MSAA
    FragColor.rgb *= 0.25f;

    // ********************************
    // sRGB conversion (if speed was a factor we would have sRGB flag as a shader specialization)
    // ********************************
//    if (FragCB.sRGB == 1)
    {
