...
layout(set = 0, binding = SHADER_DIFFUSE_TEXTURE_LOC) uniform sampler2DMS u_DiffuseTex;
...
//-----------------------------------------------------------------------------
{
    ...
    // Texture Colors
    // ********************************
    // Get base color from the multisampled color texture
    vec4 DiffuseColor = texelFetch( u_DiffuseTex, ivec2(gl_FragCoord.xy), gl_SampleID );
