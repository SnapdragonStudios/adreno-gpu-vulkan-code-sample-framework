...
VkSampleCountFlagBits gMsaa = VK_SAMPLE_COUNT_2_BIT;
...
//-----------------------------------------------------------------------------
{
    ...
    GetVulkan()->m_UseRenderPassTransform = gUseRenderPassTransform;
...
//-----------------------------------------------------------------------------
{
    ...
    gUseRenderPassTransform = gUseRenderPassTransform && GetVulkan()->GetExtRenderPassTransformAvailable();
    ...
    m_Camera.SetPosition(gCameraStartPos, gCameraStartRot);
...
//-----------------------------------------------------------------------------
{
    ...
    if( gUseSubpasses )
    {
        ...
        if (!gShaderResolve || !GetVulkan()->CreateSubpassShaderResolveRenderPass( { &PassColorFormats[0],1 }, { &PassColorFormats[1],1 }, DepthFormat, *PassColorMsaa.cbegin(), *PassColorMsaa.crbegin(), &ObjectRenderPass ))
        {
            // Drop to using Create2SubpassRenderPass (no shader resolve)
    ...
    if( gUseSubpasses )
    {
        ...
        // Blit drawable
        //
        std::map<const std::string, const Texture*> blitInputs = { {"Diffuse", &m_LinearColorRT[0].m_ColorAttachments.back() },
    ...
    else
    {
        ...
        // Tonemap drawable
        //
        std::map<const std::string, const Texture*> tonemapTex = { {"Diffuse", &m_LinearColorRT[0].m_ColorAttachments.front() } };
        ...
        // Blit drawable
        //
        std::map<const std::string, const Texture*> blitInputs = { {"Diffuse", &m_TonemapRT[0].m_ColorAttachments.back() },
                                                                   {"Overlay", &m_GuiRT[0].m_ColorAttachments.front() },
        };
        m_BlitDrawable = InitFullscreenDrawable( "Blit", blitInputs, {}, m_BlitRenderPass, 0, VK_SAMPLE_COUNT_1_BIT );
    }
...
//-----------------------------------------------------------------------------
std::unique_ptr<Drawable> Application::InitFullscreenDrawable( const char* pShaderName, const std::map<const std::string, const Texture*>& ColorAttachmentsLookup, const std::map<const std::string, const ImageInfo>& ImageAttachmentsLookup, VkRenderPass renderPass, uint32_t subpassIdx, VkSampleCountFlagBits passMsaa )
//-----------------------------------------------------------------------------
{
    //
...
//-----------------------------------------------------------------------------
{
    ...
    if( !commandBuffer.BeginRenderPass( Scissor, 0.0f, 1.0f, ClearColor, m_LinearColorRT[0].GetNumColorLayers(), true, m_LinearColorRT.m_RenderPass, false, m_LinearColorRT[0].m_FrameBuffer, VK_SUBPASS_CONTENTS_INLINE  ) )
    {
        return false;
    }
    ...
    if( !commandBuffer.BeginRenderPass( Scissor, 0.0f, 1.0f, ClearColor, 1, false, m_BlitRenderPass, true/*swapchain*/, pVulkan->m_SwapchainBuffers[bufferIdx].framebuffer, VK_SUBPASS_CONTENTS_INLINE ) )
    {
        return false;
    }
...
//-----------------------------------------------------------------------------
{
    ...
    for(const auto& ColorFormat: OutputColorFormats)
    {
        ...
        VkAttachmentDescription AttachmentDescPass1 = { 0,
            TextureFormatToVk(ColorFormat)/*format*/,
    ...
    if ( NeedsResolve && !gShaderResolve )
    {
        ...
        for( const auto& ColorFormat : OutputColorFormats )
        {
            ...
            VkAttachmentDescription AttachmentDescResolvePass1 = { 0,
                TextureFormatToVk(ColorFormat)/*format*/,
                OutputMsaa/*samples*/,
                VK_ATTACHMENT_LOAD_OP_DONT_CARE/*loadOp*/,
    ...
    VkResult RetVal = vkCreateRenderPass(GetVulkan()->m_VulkanDevice, &RenderPassInfo, NULL, pRenderPass);
...
//-----------------------------------------------------------------------------
{
    if (m_Gui)
    {
        ...
        {
            // Add our widgets
            if( GetVulkan()->GetExtRenderPassShaderResolveAvailable() )
            {
            ...
            if (GetVulkan()->GetExtRenderPassTransformAvailable())
            {
                ImGui::Checkbox("Use Renderpass Transform", &m_RequestedUseRenderPassTransform);
            }
            else
            {
                ImGui::Text("Renderpass transform extension not available");
            }
