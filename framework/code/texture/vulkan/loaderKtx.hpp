...
template<>
class TextureKtx<Vulkan> : public TextureKtxBase
{
    TextureKtx(const TextureKtx<Vulkan>&) = delete;
    TextureKtx& operator=(const TextureKtx<Vulkan>&) = delete;
public:
    TextureKtx(Vulkan& vulkan) noexcept;
    ~TextureKtx() noexcept;
    ...
    TextureVulkan LoadKtx(Vulkan& vulkan, const TextureKtxFileWrapper& textureFile, Sampler<Vulkan> sampler);
    ...
    /// @param sampler sampler that loaded texture will take OWNERSHIP of
    /// @returns a &TextureVulkan, will be empty on failure
    TextureVulkan LoadKtx(Vulkan& vulkan, AssetManager& assetManager, const char* const pFileName, Sampler<Vulkan> sampler);
