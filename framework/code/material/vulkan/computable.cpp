...
//template<>
ComputablePass<Vulkan>::ComputablePass(ComputablePass<Vulkan>&& other) noexcept
...
ComputablePass<Vulkan>::~ComputablePass()
{
}

void ComputablePass<Vulkan>::SetDispatchThreadCount(const std::array<uint32_t, 3> threadCount)
{
...
bool Computable<Vulkan>::Init()
{
    ...
    struct ImageUsage {
        ...
        ImageUsage( const ImageInfo<Vulkan>& other ) noexcept : image( other.image ), numMips( other.imageViewNumMips ), firstMip( other.imageViewFirstMip ), imageLayout( other.imageLayout) {}
    ...
    for (uint32_t materialPassIdx = 0; materialPassIdx < (uint32_t) materialPasses.size(); ++materialPassIdx)
    {
        ...
        const ShaderModule<Vulkan>& shaderModule = shaderPass.m_shaders.Get<ComputeShaderModule<Vulkan>>();
        LOGI("CreateComputePipeline: %s", shaderPass.m_shaderPassDescription.m_computeName.c_str());

        if (!mGfxApi.CreateComputePipeline(VK_NULL_HANDLE,
            pipelineLayout,
        ...
        Pipeline<Vulkan> pipeline {mGfxApi.m_VulkanDevice, vkPipeline};
        ...
        mPasses.push_back( ComputablePass<Vulkan>{ materialPass, std::move(pipeline), pipelineLayout, std::move(imageMemoryBarriers), std::move(bufferMemoryBarriers), passNeedsExecutionBarrier } );
...
//            LOGI("Buffer Binding: %s", materialPass.mShaderPass.m_shaderPassDescription.m_sets[0].m_descriptorTypes[bufferBinding.second.index].names[0].c_str());
    ...
    emitOutputBarrier(prevBufferUsages,
        [this](const VkBuffer& buffer) -> void {
            mBufferOutputMemoryBarriers.emplace_back(VkBufferMemoryBarrier{
                VK_STRUCTURE_TYPE_BUFFER_MEMORY_BARRIER,
                nullptr,
                VK_ACCESS_SHADER_WRITE_BIT, //srcAccessMask
                VK_ACCESS_SHADER_READ_BIT,  //dstAccessMask
    ...
    emitOutputBarrier(prevImageUsages,
        [this](const ImageUsage& image) -> void {
            mImageOutputMemoryBarriers.emplace_back(VkImageMemoryBarrier{
                VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER,
                nullptr,
                VK_ACCESS_SHADER_WRITE_BIT, //srcAccessMask
                VK_ACCESS_SHADER_READ_BIT,  //dstAccessMask
...
template<>
void Computable<Vulkan>::SetDispatchGroupCount(uint32_t passIdx, const std::array<uint32_t, 3>& groupCount)
{
    mPasses[passIdx].SetDispatchGroupCount(groupCount);
}

template<>
void Computable<Vulkan>::SetDispatchThreadCount(uint32_t passIdx, const std::array<uint32_t, 3>& threadCount)
{
    mPasses[passIdx].SetDispatchThreadCount(threadCount);
}

template<>
void Computable<Vulkan>::DispatchPass(CommandList<Vulkan>& cmdList, const ComputablePass<Vulkan>& computablePass, uint32_t bufferIdx) const
{
    VkCommandBuffer vkCmdBuffer = cmdList;
    ...
    vkCmdBindPipeline(vkCmdBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, computablePass.mPipeline.GetVkPipeline());

    // Bind everything the shader needs
    const std::span<const VkDescriptorSet> descriptorSets = computablePass.GetMaterialPass().GetVkDescriptorSets(bufferIdx);
    vkCmdBindDescriptorSets(vkCmdBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, computablePass.mPipelineLayout, 0, descriptorSets.size(), descriptorSets.data(), 0, nullptr);

    // Dispatch the compute task
    const auto& workgroupSettings = computablePass.GetMaterialPass().mShaderPass.m_shaderPassDescription.m_workGroupSettings;
    if (workgroupSettings.perTileDispatch)
    {
        mGfxApi.GetExtension<ExtensionLib::Ext_VK_QCOM_tile_shading>()->m_vkCmdDispatchTileQCOM(vkCmdBuffer, nullptr);
    }
    else
    {
        vkCmdDispatch(vkCmdBuffer, computablePass.GetDispatchGroupCount()[0], computablePass.GetDispatchGroupCount()[1], computablePass.GetDispatchGroupCount()[2] );
    }
}

template<>
void Computable<Vulkan>::Dispatch(CommandList<Vulkan>& cmdList, uint32_t bufferIdx, bool timers) const
{
    assert(!GetPasses().empty() && "No Computable passes, did you call Init() for the computable?");

    // Determine if we want per-tile shader mode enabling (can either dispatch per tile or just run a compute shader with user defined workgroup sizes)

    for (uint32_t passIdx =0; const auto& computablePass : GetPasses())
    {
        const int timerIdx = timers ? cmdList.StartGpuTimer( GetPassName( passIdx ) ) : -1;
        DispatchPass(cmdList, computablePass, bufferIdx % (uint32_t)computablePass.GetMaterialPass().GetVkDescriptorSets().size());
        if (timerIdx>=0)
            cmdList.StopGpuTimer( timerIdx );
        ++passIdx;
    }
}

template<>
void Computable<Vulkan>::Dispatch(CommandListBase& cmdList, uint32_t bufferIdx, bool timers) const /*override*/
{
    Dispatch(apiCast<Vulkan>(cmdList), bufferIdx, timers);
}

template<>
void Computable<Vulkan>::AddOutputBarriersToCmdList(CommandList<Vulkan>& commandList) const
{
    ...
    static_assert(std::is_same_v<VkBufferMemoryBarrier, std::remove_reference_t<decltype(computableOutputBufferBarriers.front().vkBarrier)>> );
    static_assert(std::is_same_v<VkImageMemoryBarrier, std::remove_reference_t<decltype(computableOutputImageBarriers.front().vkBarrier)>> );

    vkCmdPipelineBarrier(commandList,
                         VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,   // srcMask,
                         VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,   // dstMask,
                         0,
                         0, nullptr,
                         (uint32_t)computableOutputBufferBarriers.size(),
                         computableOutputBufferBarriers.empty() ? nullptr : (const VkBufferMemoryBarrier*) computableOutputBufferBarriers.data(),
                         (uint32_t)computableOutputImageBarriers.size(),
                         computableOutputImageBarriers.empty() ? nullptr : (const VkImageMemoryBarrier*) computableOutputImageBarriers.data());
}

template<>
void Computable<Vulkan>::AddOutputBarriersToCmdList(CommandListBase& cmdList) const /*override*/
{
    AddOutputBarriersToCmdList(apiCast<Vulkan>(cmdList));
}
